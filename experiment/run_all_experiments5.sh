...
do
	...
	do
		...
		echo $dir
		$current_path/run_mini_project.sh $dir 1 0 0 0 0 $i $intf 0 4 0 1 100
	done
	conn=$((conn*2))
done
...
do
	...
	do
		dir="$exp_name"_"$conn"_"$j"
		echo $dir
		$current_path/run_mini_project.sh $dir 1 1 0 0 0 $i $intf 0 1 0 1 100
	done
	conn=$((conn*2))
done
...
do
	...
	do
		dir="$exp_name"_"$conn"_"$j"
		echo $dir
		$current_path/run_mini_project.sh $dir 1 0 0 1 4 $i $intf 0 1 0 1 100
	done
	conn=$((conn*2))
done
