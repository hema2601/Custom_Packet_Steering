...
static int this_get_cpu(struct net_device *dev, struct sk_buff *skb,
	...
	if (flow_table) {
		...
		if(	previous_invalid(tcpu) || !previous_still_busy(tcpu) || is_overloaded(tcpu)){

			if(previous_invalid(tcpu))							stats->prevInvalid++;
			else if(!previous_still_busy(tcpu))					stats->prevIdle++;
			else{												
...
static void enqueue_busy(int tcpu)
{
...
static void remove_busy(int tcpu){
...
static void interface_mark_as_busy(int tcpu){
	...
	if(custom_toggle == 1) enqueue_busy(tcpu);

}
static void interface_mark_as_idle(int tcpu){

	if(custom_toggle == 1) remove_busy(tcpu);
...
static int my_proc_show(struct seq_file *m,void *v){
	...
	for_each_possible_cpu(i){
		...
		seq_printf(m, "%08x %08x %08x %08x %08x %08x %08x %08x %08x %08x %08x\n", i, stat.total, stat.prevInvalid, stat.prevIdle, stat.assignedToBusy, stat.noBusyAvailable, stat.targetIsSelf, stat.choseInvalid, stat.fallback, stat.prevOverloaded, stat.fromOverloaded);
	}

	for(i = 0; i < busy_histo_size; i++){
		seq_printf(m, "%08x ", busy_histo[i]);
...
static int __init init_pkt_steer_mod(void){
	...
	for_each_possible_cpu(i){
		...
		item->idle = get_cpu_idle_time(i, &item->elapsed, 0);	
	}
...
static void __exit exit_pkt_steer_mod(void){
	...
	//Restore Normal Packet Steering
	WRITE_ONCE(net_hotdata.pkt_steer_ops, &pkt_standard_ops);
		
	//Destroy IAPS flow table
	create_flow_table(1);
